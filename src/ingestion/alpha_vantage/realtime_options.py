...
def _get_symbols(endpoint: Mapping[str, Any]) -> List[str]:
    ...
    if not symbols:
        raise ConfigurationError("No symbols configured for realtime_options endpoint")
    ...
    *,
    ...
    if not isinstance(payload, Mapping) or "data" not in payload:
        LOGGER.error(
            ...
            symbol=symbol,
            keys=list(payload.keys()) if isinstance(payload, Mapping) else "<not-mapping>",
        )
        await set_heartbeat(
            redis_client,
            heartbeat_key,
