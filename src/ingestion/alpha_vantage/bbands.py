...
def _get_symbols(endpoint: Mapping[str, Any]) -> List[str]:
    ...
    if not symbols:
        raise ConfigurationError("No symbols configured for bbands endpoint")
...
async def _process_symbol(
    ...
    *,
    ...
    entry = analysis.get(first_key) if first_key else None
    if not first_key or not isinstance(entry, Mapping):
        LOGGER.error(
            "ingestion.payload_invalid",
            endpoint=ENDPOINT_SLUG,
            symbol=symbol,
        ...
        await set_heartbeat(
            ...
            status="error",
            timestamp=now.isoformat(),
            extra={"error": "malformed-bbands-entry"},
        )
        return

    required_keys = {"Real Upper Band", "Real Middle Band", "Real Lower Band"}
    missing_metrics = required_keys - set(entry)
    if missing_metrics:
        LOGGER.error(
            "ingestion.payload_invalid",
            endpoint=ENDPOINT_SLUG,
        ...
        await set_heartbeat(
            ...
            status="error",
            timestamp=now.isoformat(),
            extra={"error": "missing-bbands-components", "missing": list(sorted(missing_metrics))},
        )
        return
