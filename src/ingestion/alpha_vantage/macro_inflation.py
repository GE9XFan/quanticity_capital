"""Fetch and persist the Alpha Vantage INFLATION macro series."""
...
ENDPOINT_SLUG = "macro_inflation"


def _validate_payload(payload: Mapping[str, object], symbol: str) -> Mapping[str, object]:
    if payload.get("name") not in {"Inflation", "Inflation Rate", "Inflation - US Consumer Prices"}:
        raise PayloadValidationError(
            "INFLATION payload missing expected name",
            reason="missing-name",
    ...
    if not data or not isinstance(data, Sequence) or isinstance(data, (str, bytes)):
        raise PayloadValidationError(
            "INFLATION payload missing data series",
            reason="missing-data",
    ...
    if not isinstance(sample, Mapping):
        raise PayloadValidationError(
            "INFLATION data entry is not a mapping",
    ...
    if missing:
        raise PayloadValidationError(
            "INFLATION data entry missing required fields",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch Alpha Vantage INFLATION macro series")
    parser.add_argument(
        "--symbol",
        dest="symbols",
        action="append",
