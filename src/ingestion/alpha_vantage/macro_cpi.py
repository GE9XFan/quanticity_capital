"""Fetch and persist the Alpha Vantage CPI macro series."""
...
ENDPOINT_SLUG = "macro_cpi"


def _validate_payload(payload: Mapping[str, object], symbol: str) -> Mapping[str, object]:
    if payload.get("name") != "Consumer Price Index for all Urban Consumers":
        raise PayloadValidationError(
            "CPI payload missing expected name",
            reason="missing-name",
    ...
    if not data or not isinstance(data, Sequence) or isinstance(data, (str, bytes)):
        raise PayloadValidationError(
            "CPI payload missing data series",
            reason="missing-data",
    ...
    if not isinstance(sample, Mapping):
        raise PayloadValidationError(
            "CPI data entry is not a mapping",
    ...
    if missing:
        raise PayloadValidationError(
            "CPI data entry missing required fields",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch Alpha Vantage CPI macro series")
    parser.add_argument(
        "--symbol",
        dest="symbols",
        action="append",
