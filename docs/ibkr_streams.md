...
| Top-of-Book Quotes | awaiting-signoff | `raw:ibkr:quotes:{symbol}` | 6 | 3s | Module `src/ingestion/ibkr/quotes.py`; 2025-09-26 – Live verified with TWS, NVDA/AAPL/MSFT captured in Redis. |
...
| Execution Stream | awaiting-handshake | `stream:ibkr:executions` | ∞ | on event | Define JSON schema (order id, fill qty, price, venue). |
